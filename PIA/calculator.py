...
def newtonR(): 
    ...
    expresion = None
    mistake = None
    error = None  # Variable para error
    
    if request.method == 'POST':   
        expresion = request.form.get('expresion')
        x = float(request.form.get('x'))
        mistake = float(request.form.get('mistake'))

        if validate(expresion):
            x_sym = sp.symbols('x')
            fx = sp.sympify(expresion)
            dfx = sp.diff(fx, x_sym)  # Corregir expresión derivada

            results = iteration(x, mistake, fx, dfx)
            
            return render_template('newtonR.html', expresion=expresion, derivada=str(dfx), results=results, x=x)
            
        else:
            error = "Expresión inválida"
        
    return render_template('newtonR.html', error=error)
